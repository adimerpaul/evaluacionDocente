        ...
        <q-page>
          <div class="row">
            ...
            <div class="col-10 col-sm-4">
              <div class="text-subtitle1 text-center q-pt-xs text-grey ">ACCESO</div>
              <div class="text-h4 text-center q-pa-xs text-black text-bold fantasy">EVALUACION DOCENTE</div>
    ...
    login () {
      ...
      this.$api.post('login', {
        ...
        res.data.user.permisos.forEach(r => {
          ...
          if (r.id === 2) this.$store.booldoc = true
          if (r.id === 3) this.$store.boolmat = true
          if (r.id === 4) this.$store.boolasig = true
          if (r.id === 5) this.$store.boolest = true
          if (r.id === 6) this.$store.boolimp = true
        })
        this.$api.defaults.headers.common.Authorization = 'Bearer ' + res.data.token
        localStorage.setItem('tokenEnc', res.data.token)
      }).catch(error => {
        console.log(error)
        this.$q.notify({
